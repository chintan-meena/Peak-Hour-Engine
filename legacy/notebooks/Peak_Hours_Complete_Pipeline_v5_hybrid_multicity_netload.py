"""Reference dump of Peak_Hours_Complete_Pipeline_v5_hybrid_multicity_netload.ipynb -- code cells only, outputs stripped.
...
# ─────────────────────────────────────────────────────────────────────────────
# OPTIMIZED: Recursive net-load forecast using per-city + per-state weather
#
# Produces the SAME numbers as the original loop, but avoids recomputing
# add_net_load_features() on the entire (ever-growing, up-to-a-year-long)
# history at every single 15-min step. That full recompute — including a
# 364*96-row (annual) rolling window — is what caused the O(n^2) blowup.
#
# Strategy:
#   A) Calendar features            -> depend only on timestamp: vectorized once.
#   B) Per-city weather (raw+derived)-> depend only on KNOWN future weather
#                                       (never on the recursive Net_Load
#                                       prediction): vectorized once.
#   C) Per-city weather LAG features -> also depend only on known weather
#                                       (historical + forecast, both known
#                                       up front): vectorized once via a
#                                       single .shift() call, not one per step.
#   D) Net_Load lags & rolling stats -> the ONLY features that genuinely
#                                       depend on the recursive prediction.
#                                       Updated incrementally (O(1) per step)
#                                       via running sums instead of pandas
#                                       .rolling() over the whole frame.
# ─────────────────────────────────────────────────────────────────────────────
from collections import deque

times = pd.date_range(forecast_start, forecast_end, freq="15min")

NL_LAGS         = [1, 2, 4, 8, 96, 192, 672, 1344, 364 * 96]
NL_ROLL_WINDOWS = [96, 672, 1344, 364 * 96]
WX_LAGS         = [96, 672]

# ---- A) Calendar features for every forecast timestamp (vectorized) -------
cal = pd.DataFrame(index=times)
cal["hour"]       = cal.index.hour
cal["minute"]     = cal.index.minute
cal["dayofweek"]  = cal.index.dayofweek
cal["month"]      = cal.index.month
cal["quarter"]    = cal.index.quarter
cal["block"]      = cal.index.hour * 4 + cal.index.minute // 15
cal["is_evening"] = ((cal["hour"] >= 18) & (cal["hour"] <= 23)).astype(int)
...
cal["block_sin"]  = np.sin(2 * np.pi * cal["block"] / 96)
cal["block_cos"]  = np.cos(2 * np.pi * cal["block"] / 96)

# ---- B) Per-city weather (raw + derived) for every forecast timestamp -----
wx_now = pd.DataFrame(index=times)
for city in ALL_CITIES:
    t_col, h_col = f"{city}_Temp", f"{city}_Humidity"
    if t_col not in future_weather_15.columns:
        continue
    wx_now[t_col] = future_weather_15[t_col].reindex(times)
    wx_now[h_col] = future_weather_15[h_col].reindex(times)
    wx_now[f"{city}_cooling"]  = (wx_now[t_col] - 22).clip(lower=0)
    wx_now[f"{city}_heating"]  = (18 - wx_now[t_col]).clip(lower=0)
    wx_now[f"{city}_heat_idx"] = rothfusz_heat_index_c(wx_now[t_col], wx_now[h_col])
    wx_now[f"{city}_temp_hum"] = wx_now[t_col] * wx_now[h_col]

# ---- C) Per-city weather LAG features for every forecast timestamp --------
wx_lag = pd.DataFrame(index=times)
for city in ALL_CITIES:
    t_col, h_col = f"{city}_Temp", f"{city}_Humidity"
    if t_col not in df.columns and t_col not in future_weather_15.columns:
        continue
    hist_t = df[t_col] if t_col in df.columns else pd.Series(dtype=float)
    hist_h = df[h_col] if h_col in df.columns else pd.Series(dtype=float)
    fut_t  = future_weather_15[t_col].reindex(times) if t_col in future_weather_15.columns else pd.Series(dtype=float)
    fut_h  = future_weather_15[h_col].reindex(times) if h_col in future_weather_15.columns else pd.Series(dtype=float)
    combo_t = pd.concat([hist_t, fut_t]).sort_index()
    combo_h = pd.concat([hist_h, fut_h]).sort_index()
    for lag in WX_LAGS:
        wx_lag[f"{city}_temp_lag_{lag}"] = combo_t.shift(lag).reindex(times)
        wx_lag[f"{city}_hum_lag_{lag}"]  = combo_h.shift(lag).reindex(times)

# ---- D) Net_Load-dependent features: computed incrementally --------------
# History buffer: all historical Net_Load values, oldest -> newest.
# NOTE: we deliberately do NOT truncate this — the model uses a 364*96-row
# (annual) lag/rolling window, so the full history has to stay available.
nl_hist = list(df["Net_Load"].values)

roll_state = {}
for w in NL_ROLL_WINDOWS:
    tail = nl_hist[-w:] if len(nl_hist) >= w else nl_hist[:]
    buf = deque(tail, maxlen=w)
    roll_state[w] = {
        "buf":   buf,
        "sum":   float(np.sum(buf)) if buf else 0.0,
        "sumsq": float(np.sum(np.square(buf))) if buf else 0.0,
    }

def roll_mean_std(w):
    """Matches pandas .rolling(w).mean()/.std(): NaN (here: 0 + missing=1)
    until the window is completely full, exactly like the original code's
    default min_periods=w behavior."""
    st  = roll_state[w]
    buf = st["buf"]
    ...
    mean = st["sum"] / n
    if n > 1:
        var = max((st["sumsq"] - n * mean * mean) / (n - 1), 0.0)
...
def roll_push(value):
    for w in NL_ROLL_WINDOWS:
        st, buf = roll_state[w], roll_state[w]["buf"]
        if len(buf) == buf.maxlen:
            old = buf[0]
            st["sum"]   -= old
            st["sumsq"] -= old * old
...
for next_time in times:
    feat = {}
    feat.update(cal.loc[next_time].to_dict())
    if next_time in wx_now.index:
        feat.update(wx_now.loc[next_time].to_dict())
    if next_time in wx_lag.index:
        feat.update(wx_lag.loc[next_time].to_dict())

    # Net-load lags (use history strictly BEFORE this row)
    n = len(nl_hist)
    for lag in NL_LAGS:
        if n >= lag:
            feat[f"lag_{lag}"] = nl_hist[n - lag]
            feat[f"lag_{lag}_missing"] = 0
        else:
            feat[f"lag_{lag}"] = 0.0
            feat[f"lag_{lag}_missing"] = 1

    # Rolling stats (window ends at the row before this one, i.e. shift(1))
    for w in NL_ROLL_WINDOWS:
        mean, std, missing = roll_mean_std(w)
        feat[f"rolling_mean_{w}"] = mean
        feat[f"rolling_std_{w}"]  = std
        feat[f"rolling_mean_{w}_missing"] = int(missing)
        feat[f"rolling_std_{w}_missing"]  = int(missing)

    row_feats = pd.DataFrame([feat])
    for mf in net_features:
        if mf not in row_feats.columns:
            row_feats[mf] = 0.0
    ...
    # Feed the prediction back in for subsequent steps' lags/rolling stats
    nl_hist.append(pred)
    roll_push(pred)
