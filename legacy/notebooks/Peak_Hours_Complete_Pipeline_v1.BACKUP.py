"""Reference dump of Peak_Hours_Complete_Pipeline_v1.BACKUP.ipynb -- code cells only, outputs stripped.
...
from iex import get_trade_data
...
def load_market_from_iex_cache(market_name, start, end):
    ...
    got_days      = pd.to_datetime(combined["Date"], dayfirst=True, errors="coerce").dt.normalize().nunique()
...
def market_csv_ready(path, price_col, required_start=None, required_end=None):
    ...
    if required_start is not None and required_end is not None:
        if "Date" in df_head.columns:
            # [DAYFIRST-FIX] Date strings are DD-MM-YYYY (see
            # add_datetime_from_date_block, which already uses dayfirst=True).
            # Without dayfirst here, ambiguous dates (day <= 12) were silently
            # misread as MM-DD-YYYY, scrambling the coverage/completeness check.
            cached_dates = pd.to_datetime(df_head["Date"], dayfirst=True, errors="coerce")
...
def add_market_pressure_features(frame, purchase_col, sell_col, mcv_col, sched_col, mcp_col, out_prefix=""):
    out = frame.copy()
...
def add_all_market_pressure_features(frame):
    out = add_market_pressure_features(
        frame, "Purchase_Bid_MW", "Sell_Bid_MW", "MCV_MW",
        "Final_Scheduled_Volume_MW", "MCP_Rs_MWh", out_prefix="",
    )
    out = add_market_pressure_features(
        out, "dam_Purchase_Bid_MW", "dam_Sell_Bid_MW", "dam_MCV_MW",
        "dam_Final_Scheduled_Volume_MW", "dam_MCP_Rs_MWh", out_prefix="dam",
    )
    return out
...
RTM_MARKET_DERIVED_COLS = [
    ...
    "Cleared_Share_of_Sell", "Schedule_MCV_Gap_MW", "MCP_Rs_kWh",
]
...
PEAK_START_BLOCK = int(_crossing.index.min()) if not _crossing.empty else _trough_block
...
print(
    ...
    f"{PEAK_START_BLOCK} ({block_to_time(PEAK_START_BLOCK)}) -- peak-hour "
    f"candidate windows are now restricted to start at or after this block."
)
